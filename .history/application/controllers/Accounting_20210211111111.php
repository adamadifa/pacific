...
class Accounting extends CI_Controller
{
  ...
  function view_pembelian()
  {
    ...
    $bulan          = $this->input->post('bulan');
    $tahun          = $this->input->post('tahun');
    $data['detailpembelian'] = $this->Model_accounting->detailpembelian($kode_akun, $bulan, $tahun)->result();
    $data['detailpembelian'] = $this->Model_accounting->pembelian($kode_akun, $bulan, $tahun)->result();
  ...
  function view_penjualan()
  {
    ...
    if ($kode_akun == "1-1401") {
      ...
      $data['penjualannet']   = $this->Model_accounting->penjualannet($bulan, $tahun)->result();
      $data['totalretur']     = $this->Model_accounting->totalretur($bulan, $tahun)->row_array();
      $this->load->view('accounting/view_penjualan', $data);
    } else if ($kode_akun == "4-2101") {
      $data['kode_akun']        = $this->input->post('kode_akun');
      $data['returpenjualan']   = $this->Model_accounting->returpenjualan($bulan, $tahun)->result();
      $this->load->view('accounting/view_penjualan', $data);
    } else if ($kode_akun == "4-2201") {
      $data['kode_akun']           = $this->input->post('kode_akun');
      $data['potonganpenjualan']   = $this->Model_accounting->potonganpenjualan($bulan, $tahun)->result();
      $this->load->view('accounting/view_penjualan', $data);
    } else if ($kode_akun == "4-2202") {
      $data['kode_akun']           = $this->input->post('kode_akun');
      $data['penypenjualan']   = $this->Model_accounting->penypenjualan($bulan, $tahun)->result();
      $this->load->view('accounting/view_penjualan', $data);
    } else {
      $data['kode_akun']      = $this->input->post('kode_akun');
