...
<table class="datatable3">
  ...
  <tr style="font-size:14px; background-color: black; color:white">
    ...
    <td align="right"><?php echo number_format(($harganet['bruto_AB'] - (($harganet['qty_AB'] / $isipcs_ab) / $ratiopeny) - $harganet['diskon_AB']) / ($harganet['qty_AB'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_AR'] - (($harganet['qty_AR'] / $isipcs_ar) / $ratiopeny) - $harganet['diskon_AR']) / ($harganet['qty_AR'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_AS'] - (($harganet['qty_AS'] / $isipcs_as) / $ratiopeny) - $harganet['diskon_AS']) / ($harganet['qty_AS'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_BB'] - (($harganet['qty_BB'] / $isipcs_bb) / $ratiopeny) - $harganet['diskon_BB']) / ($harganet['qty_BB'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_BBP'] - (($harganet['qty_BBP'] / $isipcs_bbp) / $ratiopeny) - $harganet['diskon_BBP']) / ($harganet['qty_BBP'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_CG'] - (($harganet['qty_CG'] / $isipcs_cg) / $ratiopeny) - $harganet['diskon_CG']) / ($harganet['qty_CG'] / $isipcs_ab), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format(($harganet['bruto_CGG'] - (($harganet['qty_CGG'] / $isipcs_cgg) / $ratiopeny) - $harganet['diskon_CGG']) / ($harganet['qty_CGG'] / $isipcs_ab), '0', '', '.'); ?></td>
  ...
  <tr style="font-size:14px; background-color:black; color:white">
    <?php
    $netjual_AB = ($harganet['bruto_AB'] - $harganet['diskon_AB'] - $peny_AB - $netretur_AB) / ($harganet['qty_AB'] / $isipcs_ab);
    $netjual_AR = ($harganet['bruto_AR'] - $harganet['diskon_AR'] - $peny_AR - $netretur_AR) / ($harganet['qty_AR'] / $isipcs_ar);
    $netjual_AS = ($harganet['bruto_AS'] - $harganet['diskon_AS'] - $peny_AS - $netretur_AS) / ($harganet['qty_AS'] / $isipcs_as);
    $netjual_BB = ($harganet['bruto_BB'] - $harganet['diskon_BB'] - $peny_ABB - $netretur_BB) / ($harganet['qty_BB'] / $isipcs_bb);
    $netjual_BBP = ($harganet['bruto_BBP'] - $harganet['diskon_BBP'] - $peny_BBP - $netretur_BBP) / ($harganet['qty_BBP'] / $isipcs_bbp);
    $netjual_CG = ($harganet['bruto_CG'] - $harganet['diskon_CG'] - $peny_CG - $netretur_CG) / ($harganet['qty_CG'] / $isipcs_cg);
    $netjual_CGG = ($harganet['bruto_CGG'] - $harganet['diskon_CGG'] - $peny_CGG - $netretur_CGG) / ($harganet['qty_CGG'] / $isipcs_cgg);
    $netjual_CG5 = ($harganet['bruto_CG5'] - $harganet['diskon_CG5'] - $peny_CG5 - $netretur_CG5) / ($harganet['qty_CG5'] / $isipcs_cg5);
  ...
  <tr style="font-size:14px; background-color:black; color:white">
    ...
    <td align="right"><?php echo number_format($exclude_BB - ($harganet['diskon_BB'] / ($harganet['qty_BB'] / $isipcs_bb)), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format($exclude_BBP - ($harganet['diskon_BBP'] / ($harganet['qty_BBP'] / $isipcs_bbp)), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format($exclude_CG - ($harganet['diskon_CG'] / ($harganet['qty_CG'] / $isipcs_cg)), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format($exclude_CGG - ($harganet['diskon_CGG'] / ($harganet['qty_CGG'] / $isipcs_cgg)), '0', '', '.'); ?></td>
    <td align="right"><?php echo number_format($exclude_CG5 - ($harganet['diskon_CG5'] / ($harganet['qty_CG5'] / $isipcs_cg5)), '0', '', '.'); ?></td>
